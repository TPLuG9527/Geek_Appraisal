...
![avatar](https://files.catbox.moe/6mdbc8.png)
![avatar](https://files.catbox.moe/q6eu80.png)
![avatar](https://files.catbox.moe/x2ecp3.jpg)
...
//当叶子节点有n个，路径就是n-1;有n-1个路径，就有n-1个非叶子结点，加起来共2n-1
2. 该代码可以对大写字母的字符串进行huffman编码，如果要读小写字母字符串进行编码，应该如何修改？
/*(str[i] == (char)(0x41 + j)*/改为/*(str[i] == (char)(0x61 + j)*/
如果想要对同时具有大写和小写字母的字符串都进行编码呢（写出思路即可）？
/*   for(int j = 0; j < 27; j++) */改为   /*for(int j = 0; j < 27||（j>32&&j<57); j++) */
