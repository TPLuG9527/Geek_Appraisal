...
int main()
{
    ...
    for(int i = 0; i < 27; i++)
    {
        if((arr_big[i])!=0)
        {
            arr_small[count2++] = arr_big[i];	/*将arr_big[i]中的值存储到arr_small[i]中 并一一对应*/
        }
    }
    ...
    {
        int length = 0;
        if(huffman[i][1] == -1 && huffman[i][2] == -1)  //当该节点是叶子节点
        {
...
void huffmanTree(int w[],int huffman[][4],int n)
{
    ...
    /*一开始这几个哥们啥也不是 给个0也行，给个-1也行，不影响后续就好*/
    for(int i = 0; i < n; i++)
    {
        huffman[i][3] = w[i];
    }
...
void findMin(int ii[],int huffman[][4],int n)
{
    ...
    for(int i = 0; i < 2 * n - 1; i++)
    {
        if(huffman[i][3] == -1 && huffman[i][0] == -1)  /*没有按要求输入时破坏循环*/
        {
            break;
        }
        if(huffman[i][3] < min1 && huffman[i][0] == -1)  /*当叶子没有爹时通过不断比较找到最小权重（左孩子）*/
        {
            min1 = huffman[i][3];
            ii[0] = i;
        }
    ...
    for(int k=0;k<2*n-1;k++)
 	...
 	if(huffman[k][3]<min2&&k!=ii[0]&&huffman[k][0]==-1)
 	{
 		min2=huffman[k][3];
 		ii[1]=k;
	 }
...
void HuffmanCode(int i,int huffman[][4],int n)
{
    ...
    int father = huffman[current][0]; //定义当前结点的父节点
    int start=0;    //每次编码的位置，初始为编码倒数位置
    int first,last;     //char数组的头部和尾部
 
	
		
			//判断当前结点的父结点左子树是否为当前结点
		while(father != -1)
  	  {
        if(huffman[father][1]==current)		//子结点是父结点的左子树，编码为0
        {
    		code[start]='0';    	
		}
        else		//子结点是父结点的右子树，编码为1
			{
				code[start]='1';
			}
